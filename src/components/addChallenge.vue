<template>
  <div class="modal-box" >
    ...
    <div id="New" class="">
      <form action="#"  @submit.prevent="addChallenge">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <button type="submit" class="btn float-right btn-primary my-2" :disabled="addLoading">
    ...
    <div id="Import" class="hidden">
      <div class="p-4 my-4 text-sm rounded-lg bg-gray-800 text-blue-400" role="alert">
        <span class="font-bold">New Update!</span> Challenge start/finish date and challenge title are imported from your post.
      </div>
      ...
      <form action="#" @submit.prevent="importChallenge">
        ...
        <div class="grid grid-cols-2 gap-4">
          <a href="#" class="btn float-left btn-error my-2" @click="cancel">Cancel</a>
          <button type="submit" class="btn float-right btn-primary my-2" :disabled="importLoading">
            {{ importLoading ? 'Saving...' : 'Save' }}
          </button>
        </div>
      </form>
    </div>
  ...
  methods: {
    ...
    async importChallenge() {
      ...
      try {
        ...
        const index = this.post_link.indexOf('/comment/');
        if (index > -1) {
          this.thread_link =  this.post_link.substring(0, index);
        }
        ...
        if (fulfilments === null) {
          ...
          await fetch(url, options).then(handleResponse)
              .then(async (result) => {
                ...
                let dateRegex = /(?:^|[^\w])([Y0-9]{4})-([M0-9]{2})-([D0-9]{2})(?:[^\w]|$)/
                let count = 0;
                ...
                for (let i = 0; i < line.length; i++) {
                  ...
                  else if (line[i].match(dateRegex)) {
                    if (line[i].startsWith("Challenge Start Date:")) {
                      startDate = line[i].match(dateRegex)[0]
                    }

                    if (line[i].startsWith("Challenge Finish Date:")) {
                      endDate = line[i].match(dateRegex)[0]
                    }
                  }
                  else if (line[i].startsWith("# __")) {
                    let temp = line[i].split("__");
